...
class InputEvent {
	public:
		...
		template<class C, typename N = int> void execSlot(C Class, N n_ev) {
			auto it = Class->slots.find({event, events[n_ev][0], events[n_ev][1], events[n_ev][2]});
			if(it != Class->slots.end())
				it->second();
		}

		void getEvents();
		static void setEvent(GLFWwindow* window, const int type, const int sign, const int action);
